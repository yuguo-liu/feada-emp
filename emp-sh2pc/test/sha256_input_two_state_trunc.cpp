...
int main(int argc, char** argv) {
    ...
    setup_plain_prot(true, "sha256_i_2_share_state_2_full_msg_o_2_share_hash_1_key_hash.txt");

    Integer state_alice {256, 0, ALICE};
    Integer hash_val_1 {512, 0, ALICE};
    Integer mask_alice {384, 0, ALICE};

    Integer state_bob {256, 0, BOB};
    Integer hash_val_2 {512, 0, BOB};
    Integer mask_bob {384, 0, BOB};
    ...
    Bit server_key[192];
    for (int i = 192 / 8; i < 256 / 8; i++) {
        for (int j = 0; j < 8; j++) {
            server_key[i * 8 + j - 192] = hash_val_res_1[i * 8 + (7 - j)];
        }
    }

    for (int i = 0; i < 128/ 8; i++) {
        for (int j = 0; j < 8; j++) {
            server_key[64 + i * 8 + j] = hash_val_res_2[i * 8 + (7 - j)]; 
        }
    }

    Bit hash_server_key[256];
    sha256((block*) server_key, (block*) hash_server_key, 192);

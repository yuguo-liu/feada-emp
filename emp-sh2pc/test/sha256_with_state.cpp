...
int main(int argc, char** argv) {
    ...
    setup_plain_prot(true, "sha256_output_state.txt");

    Integer alice_share {512, 0, ALICE};
    Integer alice_state {256, 0, ALICE};
    Integer bob_share {512, 0, BOB};

    Bit hash_a_plus_b[256];
    Bit output_state[256];
    sha256_exec((alice_share + bob_share), hash_a_plus_b, alice_state, output_state);

    for (int i = 0; i < 256 / 8; i++) {
        for (int j = 0; j < 8; j++){
            hash_a_plus_b[i * 8 + (7 - j)].reveal();
        }
    }

...
int main(int argc, char** argv) {
    ...
    setup_plain_prot(true, "predicate_ghash_" + std::to_string(BLOCK) + ".txt");
    ...
    Integer bob_dummy {640, 0, BOB};            // align the input of alice

    Integer bob_dummies[BLOCK];                 // align the input of alice
    for (int i = 0; i < BLOCK; i++) {
        bob_dummies[i] = Integer {128, 0, BOB};
    }

    Integer ghash {128, 0, ALICE};              // ghash of associated data (computed locally by Alice)

    Integer plaintexts_wo_sensitive[BLOCK];     // plaintexts held by Alice
    for (int i = 0; i < BLOCK; i++) {
    ...
    for (int i = 0; i < BLOCK; i++) {
        block* one_plaintext = (block*) (plaintexts[i] ^ plaintexts_wo_sensitive[i]).bits.data();
        for (int j = 0; j < 128; j++) {
    ...
    for (int i = 0; i < BLOCK; i++) {
        ciphertexts[i] = (ciphertexts[i] & mask_int) ^ bob_mask;
    }

    ghash = (ghash & mask_int) ^ bob_mask;

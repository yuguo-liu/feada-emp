...
int main(int argc, char** argv) {
    ...
    setup_plain_prot(true, "general_dec_ghash_" + std::to_string(BLOCK) + ".txt");
    ...
    Integer tag {128, 0, BOB};                  // tag held by Bob

    Integer len_c_data {128, 0, BOB};           // length of ciphertext

    Integer bob_mask {128, 0, BOB};             // mask of bob

    Integer bob_padding_mask {128, 0, BOB};            // align the input of alice
    ...
    // padding mask the pad (or bob can learn the pad from the decrypted cipher)
    pads[BLOCK - 1] = pads[BLOCK - 1] & bob_padding_mask;
    ...
    for (int i = 0; i < BLOCK; i++) {
        plaintexts[i] = (plaintexts[i] & mask_int) ^ bob_mask;
    }
    ...
    for (int i = 0; i < BLOCK; i++) {
        string p = plaintexts[i].reveal<string>();
        cout << "plaintext block " << i << ": " << p << endl;
    }

...
int main(int argc, char** argv) {
    ...
    setup_plain_prot(true, "sha256_i_2_share_msg_1_state_o_hash_state.txt");
    ...
    sha256_exec((share_alice ^ share_bob), hash_val_res, state, output_state);

    for (int i = 0; i < 256 / 8; i++) {
        for (int j = 0; j < 8; j++){
            hash_val_res[i * 8 + (7 - j)].reveal();
        }
    }

    for (int i = 0; i < 256; i++) {
        output_state[i].reveal();
    }

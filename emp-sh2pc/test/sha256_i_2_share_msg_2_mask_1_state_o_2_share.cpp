...
int main(int argc, char** argv) {
    ...
    setup_plain_prot(true, "sha256_i_2_share_msg_2_mask_1_state_o_2_share_addition.txt");

    Integer mask_alice {256, 0, ALICE};
    Integer share_alice {512, 0, ALICE};

    Integer state {256, 0, ALICE};

    Integer mask_bob {256, 0, BOB};
    Integer share_bob {512, 0, BOB};

    Integer mask = mask_alice ^ mask_bob;
    Bit hash_val_res[256];
    Bit output_state[256];

    sha256_exec(mod_add(share_alice, share_bob), hash_val_res, state, output_state);

    for (int i = 0; i < 256 / 8; i++) {
        for (int j = 0; j < 8; j++){
            (hash_val_res[i * 8 + (7 - j)] ^ mask.bits[i * 8 + j]).reveal();
        }
    }

    for (int i = 0; i < 256; i++) {
        (output_state[i] ^ mask.bits[i]).reveal();
    }
...
Integer mod_add(Integer a, Integer b) {
    ...
    vector<Bit> modular_bits;
    for (int i = 0; i < 8; i++) {
        word32 tmp = modular[i];
        for (int j = 0; j < 32; j++) {
            modular_bits.push_back((tmp & 1) != 0 ? one : zero);
            tmp >>= 1;
        }
    }

    Integer modular_i (modular_bits);

    Integer add = a + b;

    Bit to_mod = (add < Integer(1, 0, PUBLIC)) | (add > modular_i);

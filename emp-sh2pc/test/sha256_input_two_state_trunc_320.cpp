...
void sha256_exec(Integer input_int, Bit* output, Integer state, Bit* output_state);
void sha256_with_state(block *input, block *output, block *state, int input_len, Bit* output_state);
...
int main(int argc, char** argv) {
    ...
    setup_plain_prot(true, "sha256_i_2_share_state_2_full_msg_o_2_share_hash_1_key_hash_320.txt");

    Integer state_alice {256, 0, ALICE};
    Integer hash_val_1 {512, 0, ALICE};
    Integer mask_alice {320, 0, ALICE};

    Integer state_bob {256, 0, BOB};
    Integer hash_val_2 {512, 0, BOB};
    Integer mask_bob {320, 0, BOB};

    Integer mask = mask_alice ^ mask_bob;

    Bit hash_val_res_1[256];
    Bit output_state_1[256];
    sha256_exec(hash_val_1, hash_val_res_1, (state_alice ^ state_bob), output_state_1);

    Bit hash_val_res_2[256];
    Bit output_state_2[256];
    sha256_exec(hash_val_2, hash_val_res_2, (state_alice ^ state_bob), output_state_2);

    Bit server_key[128];
    for (int i = 128 / 8; i < 256 / 8; i++) {
        for (int j = 0; j < 8; j++) {
            server_key[i * 8 + j - 128] = hash_val_res_1[i * 8 + (7 - j)];
        }
    }

    // for (int i = 32 / 8; i < 64 / 8; i++) {
    //     for (int j = 0; j < 8; j++) {
    //         server_key[96 + i * 8 + j] = hash_val_res_2[i * 8 + (7 - j)]; 
    //     }
    // }

    Bit hash_server_key[256];
    sha256((block*) server_key, (block*) hash_server_key, 128);

    for (int i = 0; i < 256 / 8; i++) {
        for (int j = 0; j < 8; j++) {
            (hash_val_res_1[i * 8 + (7 - j)] ^ mask.bits[i * 8 + j]).reveal();
        }
    }

    for (int i = 0; i < 64 / 8; i++) {
        for (int j = 0; j < 8; j++) {
            (hash_val_res_2[i * 8 + (7 - j)] ^ mask.bits[256 + i * 8 + j]).reveal();
        }
    }

    for (int i = 0; i < 256 / 8; i++) {
        for (int j = 0; j < 8; j++) {
            hash_server_key[i * 8 + (7 - j)].reveal();
        }
    }

    // for (int i = 0; i < 256 / 8; i++) {
    //     for (int j = 0; j < 8; j++) {
    //         hash_val_res_1[i * 8 + (7 - j)].reveal();
    //     }
    // }

    // for (int i = 0; i < 128 / 8; i++) {
    //     for (int j = 0; j < 8; j++) {
    //         hash_val_res_2[i * 8 + (7 - j)].reveal();
    //     }
    // }
...
void sha256_exec(Integer input_int, Bit* output, Integer state, Bit* output_state) {
    sha256_with_state((block*) input_int.bits.data(), (block*) output, (block*) state.bits.data(), 512, output_state);
    // print_hash(output);
}
...
void sha256_with_state(block *input, block *output, block *state, int input_len, Bit* output_state) {
    ...
    for (int i = 0; i < 512; i++) {
        input_to_sha256_circuit[i] = input_new[i];
    }

    for (int i = 0; i < 256; i++) {
        input_to_sha256_circuit[512 + i] = state[i];
    }
    ...
    for (int i = 0; i < 256; i++) {
        output_state[i] = output_from_sha256_circuit[i];
    }

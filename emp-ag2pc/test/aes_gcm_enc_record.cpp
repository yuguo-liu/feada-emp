...
//	io->set_nodelay();
    ...
    if (party == ALICE) {
        ...
        string in = io_bob_plaintext + io_bob_len_c_data + io_bob_mask + io_bob_padding_mask + io_bob_dummy + io_alice_ghash + io_alice_pads + io_alice_h + io_alice_ek_counter0 + io_alice_len_a_data;
        cout << party << ": " << in.length() << endl;
        cout << party << ": " << in << endl;

        res = ag2pc_exec(party, io, circuit_file_location + "/AES/general_enc_ghash_" + to_string(BLOCK) + ".txt", in);

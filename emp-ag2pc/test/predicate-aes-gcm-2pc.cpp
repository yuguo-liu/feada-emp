...
int main(int argc, char** argv) {
    ...
    if (argc != 15) {
        cerr << "Parameter error: expect 15 get " << argc << endl;
        exit(1);
    }
    ...
    string bob_m                    = argv[3];
    string bob_key_share            = argv[4];
    string bob_iv_share             = argv[5];
    
    string alice_m                  = argv[6];
    string alice_auth_data          = argv[7];
    string alice_key_share          = argv[8];
    string alice_iv_share           = argv[9];
    string r_com                    = argv[10];
    string commitment               = argv[11];

    int len_c_i = atoi(argv[12]);
    int len_a_i = atoi(argv[13]);
    ...
    string len_c = int_to_hex_16(bob_m.length() * 4);
    string len_a = int_to_hex_16(alice_auth_data.length() * 4);

    if (party == BOB){
        io_bob_plaintext         = pad_hex_string(bob_m);
        ...
        io_bob_dummy             = string((auth_block + 2) * 32, '0');
        
        io_alice_plaintext       = string(plain_block * 32, '0');
        ...
        string c = ag2pc_exec(party, io, circuit_file_location+"AES/predicate-standard-aes-gcm-" + std::to_string(plain_block) + ".txt", in);

        cout << c.substr(0, 1) << endl;
        cout << c.substr(1, (int) len_c_i / 4) << endl;
        cout << c.substr(1 + plain_block * 32, 32) << endl;
    } else {
        io_bob_plaintext         = string(plain_block * 32, '0');
        io_bob_dummy             = string((auth_block + 2) * 32, '0');

        io_alice_plaintext       = pad_hex_string(alice_m);

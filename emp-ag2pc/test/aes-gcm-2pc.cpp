...
int main(int argc, char** argv) {
    ...
    if (argc != 12) {
        cerr << "Parameter error: expect 12 get " << argc << endl;
        exit(1);
    }
    ...
    string alice_auth_data          = argv[6];
    string alice_key_share          = argv[7];
    string alice_iv_share           = argv[8];

    int len_c_i = atoi(argv[9]);
    int len_a_i = atoi(argv[10]);
    ...
    if (party == BOB) {
        ...
        io_alice_dummy           = string((plain_block - auth_block + 1) * 32, '0');
        ...
        string c = ag2pc_exec(party, io, circuit_file_location+"AES/aes-gcm-" + std::to_string(plain_block) + ".txt", in);
